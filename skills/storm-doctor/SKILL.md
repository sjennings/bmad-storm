...
| `capabilities` | Write-role deny lists still cover every known lifecycle/control and Linear-mutation tool. Static audit cannot observe the live MCP server — after any Polytoken or Linear MCP update, enumerate the server's mutation tools in an operator session and confirm the deny lists still cover them. |
